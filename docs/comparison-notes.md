...
- 所感:
  ...
  - GitHub Actions では k6 の benchmark workflow を追加し、REST / gRPC を直列実行して JSON summary、summary.txt、summary.md、アプリケーションログを artifact として回収できる状態にした
  - 通常の k6 CI では artifact を正本とし、Grafana は観測確認用途に分ける方針とした
...
- Keploy については、OpenTelemetry Collector で `keploy-rest.log` / `keploy-grpc.log` を Loki に取り込み、Grafana で回帰確認ログを BFF / backend のログ、Prometheus の request count と並べて確認できる状態にした
- Keploy 用の Grafana dashboard として `Keploy 回帰確認 Overview` を追加し、complete runs、passed / failed testcase lines、BFF / backend request count、Keploy / BFF / backend logs を 1 画面で見られるようにした
- Keploy の complete runs は Loki 上で `COMPLETE TESTRUN SUMMARY` を含むログ行数から集計し、passed / failed testcase lines は testcase の pass/fail 行から集計する構成にした
- BFF requests と backend request count は Prometheus の `http_server_requests_seconds_count` を selected range で `increase` した値を採用し、Keploy 実行中にどの endpoint がどの程度叩かれたかを把握できるようにした
...
- 運用上の前提:
  ...
  - GitHub Actions workflow 自体は追加済みである

#### 5.2 Keploy を使った Grafana 観測確認の位置づけ

Keploy についても、通常 CI とは別に、Grafana 上で回帰確認の様子を観測できる手動 workflow を追加した。

- workflow:
  - `keploy-grafana-observability.yml`
  - `workflow_dispatch` で `both` / `rest` / `grpc` を選択可能
  - observability stack を起動したうえで Keploy を実行し、最後に Grafana / Prometheus / docker compose の snapshot を artifact として回収する

- 可視化の考え方:
  - Keploy 自体を Prometheus メトリクスへ直接変換するのではなく、Keploy の実行ログを Loki に取り込んで Grafana の logs / stat panel で集計する
  - アプリケーション側の request count は既存の Prometheus メトリクスを使う
  - これにより、Keploy 実行結果、BFF / backend のログ、BFF / backend の request count を 1 画面で相関確認できる
...
- 運用上の整理:
  - 通常の回帰判定は引き続き Keploy CI の success / failure を正本とする
  - Grafana 側は「なぜ失敗したか」「どの endpoint がどれだけ叩かれたか」「BFF / backend のログに何が出ていたか」を確認する補助用途として扱う
  - これにより、Keploy も k6 と同様に「正本」と「観測確認」を分けた整理にできた
...
- 追加実施内容:
  - k6 の GitHub Actions benchmark workflow を追加し、`workflow_dispatch` で `both` / `rest` / `grpc` と `both` / `users` / `orders` を選択可能にした
  - k6 benchmark は REST / gRPC を直列実行し、各 mode / scenario ごとの `summary.json`、`summary.txt`、全体の `summary.md`、アプリケーションログを artifact に回収する構成にした
  - k6 は通常の benchmark workflow では Grafana 連携を必須にせず、artifact 正本の比較フローに寄せた
  - Keploy についても、observability stack を起動したうえで実行する手動 workflow を追加し、Grafana / Prometheus / Loki 上で回帰確認の様子を観測できるようにした
  - OpenTelemetry Collector に `keploy-rest.log` / `keploy-grpc.log` を取り込む filelog receiver を追加し、Keploy 実行ログを Loki に送る構成を追加した
  - Grafana dashboard `Keploy 回帰確認 Overview` を追加し、Keploy complete runs、passed / failed testcase lines、BFF request count、backend request count、Keploy / BFF / backend logs を 1 画面に集約した
  - Keploy observability workflow では Grafana health、Prometheus metric names、docker compose ps / logs を artifact に回収するようにした

- 追加結果概要:
  - k6 は CI 上で REST / gRPC を同条件で直列実行し、比較に使う summary とログを artifact として安定回収できる状態になった
  - Keploy は通常 CI に加え、Grafana 上で実行ログとアプリケーションログ、request count を横断確認できる状態になった
  - `Keploy 回帰確認 Overview` では complete runs、passed testcase lines、failed testcase lines、BFF / backend request count、Keploy / BFF / backend logs を確認できた
  - Grafana 上の確認では、Keploy complete runs は 4、passed testcase lines は 32、failed testcase lines は 0 を確認した
  - BFF request count では `GET /actuator/prometheus` の scrape が多く含まれる一方、`POST /api/orders` の 201 / 400、`GET /api/users/{id}` の 200 / 404 が selected range で観測できた
  - Backend request count でも同様に、selected range で backend 側の request 分布を確認できた

- 追加の気づき:
  - k6 は「比較値の正本は artifact」、Grafana は「観測確認」という分離が明確になった
  - Keploy も「回帰判定の正本は workflow / report」、「失敗原因や周辺挙動の確認は Grafana」という役割分担に整理できた
  - Prometheus の request count には `actuator/prometheus` の scrape が多く含まれるため、必要に応じて dashboard 側で除外クエリを別途用意した方が、API 本体の挙動を見やすい
  - `Keploy 回帰確認 Overview` は PoC 段階のトラブルシュートには有効だが、将来的には run label や workflow run 単位で絞り込めるようにするとさらに見やすくなる
...
- PoC 終盤では、Keploy / k6 / Grafana の役割分担を次のように整理できた
  - Bruno:
    - 外部 API の手動確認と正解決め
