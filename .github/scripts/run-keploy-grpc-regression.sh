...
ensure_test_set_layout() {
    ...
    local test_set_dir="$BFF_DIR/keploy/${test_set_name}"
...
test_set_contains_generic_mocks() {
    local test_set_name="$1"
    local mocks_file="$BFF_DIR/keploy/${test_set_name}/mocks.yaml"
...
select_active_test_set() {
    ...
    if test_set_contains_generic_mocks "$PRIMARY_TEST_SET"; then
        ...
        print_info "${FALLBACK_TEST_SET} を初回 green 用の clean smoke asset として使用します。"
        ensure_test_set_layout "$FALLBACK_TEST_SET"
        ACTIVE_TEST_SET="$FALLBACK_TEST_SET"
        return 0
    fi
...
print_active_test_set_summary() {
    local test_set_dir="$BFF_DIR/keploy/${ACTIVE_TEST_SET}"
