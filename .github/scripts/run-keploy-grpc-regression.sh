...
# 実装定義に合わせて、HTTP health は 19091、gRPC 待受は 29090 を確認する。
...
# BFF のファイルログ出力先も明示し、CI 失敗時の切り分けをしやすくする。
BFF_COMMAND='bash -lc '"'"'
...
# Keploy 実行後に BFF のヘルス確認も一度行い、起動に成功していたことを明示しておく。
wait_for_http "bff health" "http://127.0.0.1:8080/health" "$BFF_READY_TIMEOUT"
